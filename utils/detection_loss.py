...
class DetectionLoss(nn.Module):
    def __init__(self, config):
        ...
        self.config = config
        # 损失权重
        self.lambda_box = 5.0  # 边界框回归损失权重
        self.lambda_obj = 1.0  # 目标性损失权重
        self.lambda_cls = 1.0  # 分类损失权重
        self.lambda_temporal = 0.1  # 时序一致性损失权重
        self.lambda_feature = 0.05  # 特征融合损失权重

    def compute_box_loss(self, pred_boxes, target_boxes, obj_mask):
        """
        计算边界框回归损失
        pred_boxes: [B, A*4, H, W]
        target_boxes: [B, A*4, H, W]
        obj_mask: [B, A, H, W]
        """
        ...
        return loss / (obj_mask.sum() + 1e-16)

    def compute_obj_loss(self, pred_obj, target_obj, obj_mask):
        """
        计算目标性损失
        pred_obj: [B, A, H, W]
        target_obj: [B, A, H, W]
        obj_mask: [B, A, H, W]
        """
        bce = F.binary_cross_entropy_with_logits(pred_obj, target_obj, reduction='none')
        return (bce * obj_mask).sum() / (obj_mask.sum() + 1e-16)

    def compute_cls_loss(self, pred_cls, target_cls, obj_mask):
        """
        计算分类损失
        pred_cls: [B, A*C, H, W]
        target_cls: [B, A*C, H, W]
        obj_mask: [B, A, H, W]
        """
        ...
        return (bce * obj_mask.unsqueeze(1)).sum() / (obj_mask.sum() + 1e-16)
    ...
    def compute_feature_fusion_loss(self, temporal_features, original_features):
        """
        计算特征融合损失
        temporal_features: [B, C, H, W] - 时序融合后的特征
        original_features: [B, T, C, H, W] - 原始特征序列
        """
        # 确保时序特征保留了原始特征的关键信息
        original_mean = original_features.mean(dim=1)  # [B, C, H, W]
        return F.mse_loss(temporal_features, original_mean)
    ...
    def forward(self, predictions, targets, features, temporal_features):
        """
        计算总损失
        predictions: [p1, p2, p3] - 多尺度预测结果
        targets: [t1, t2, t3] - 多尺度目标
        features: [B, T, C, H, W] - 原始特征序列
        temporal_features: [B, C, H, W] - 时序融合后的特征
        """
        total_loss = 0
